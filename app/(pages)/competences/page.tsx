...
export default function CompetencesClient() {
  ...
  const [filieres, setFilieres] = useState<Filiere[]>([]); // supposons que tu aies un fetch
  ...
  const fetchCompetences = async () => {
    try {
      ...
      setCompetences(data.map((c: any) => ({ ...c, createdAt: new Date(c.createdAt) })));
    } catch {
  ...
  const fetchFilieres = async () => {
    try {
      // Simule récupération filières, remplacer par fetch réel
      const data: Filiere[] = [
  ...
  const openEditPopup = (c: Competence) => {
    ...
    setForm({ nom: c.nom, maxScore: c.maxScore, coefficient: c.coefficient });
    ...
    <div className="relative max-w-7xl mx-auto px-6 py-8">
        ...
        <table className="table w-full">
          <thead className="bg-base-200 text-sm">
            <tr>
              ...
              <th>Nom</th>
              <th>
                <div className="flex items-center gap-1 cursor-pointer select-none" onClick={toggleFiliereSort}>
                  Filière
                ...
                <tr key={c.id}>
                  ...
                  <td className="text-center">
                    <div className="flex justify-center gap-2">
                      <button className="btn btn-xs btn-outline btn-warning" onClick={() => openEditPopup(c)}>
                        <LucideEdit2 size={14} />
                      </button>
                      <button className="btn btn-xs btn-outline btn-error" onClick={() => handleDeleteCompetence(c.id)}>
      ...
      {popupOpen && (
        <dialog className="modal modal-open">
          <form className="modal-box rounded-3xl max-w-lg w-full p-8 flex flex-col gap-5 relative" onSubmit={handleAddCompetence}>
            ...
            <input name="nom" placeholder="Nom" className="input input-bordered w-full" required
              value={form.nom} onChange={(e) => setForm({...form, nom: e.target.value})} />

            <input type="number" name="maxScore" placeholder="Score max" className="input input-bordered w-full" required
              value={form.maxScore} onChange={(e) => setForm({...form, maxScore: parseFloat(e.target.value)})} />

            <input type="number" name="coefficient" placeholder="Coefficient" className="input input-bordered w-full"
              value={form.coefficient} onChange={(e) => setForm({...form, coefficient: parseFloat(e.target.value)})} />

            <Select options={filiereOptions} value={selectedFiliere} onChange={setSelectedFiliere} placeholder="Sélectionner une filière" />
      ...
      {editPopupOpen && selectedCompetence && (
        <dialog className="modal modal-open">
          <form className="modal-box rounded-3xl max-w-lg w-full p-8 flex flex-col gap-5 relative" onSubmit={handleUpdateCompetence}>
