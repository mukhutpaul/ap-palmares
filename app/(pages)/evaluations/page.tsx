...
import { getEtudiants, syncEtudiants } from "@/app/actions/etudiantsActions";
...
export default function EvaluationsClient() {
  ...
  const [studentFiliere, setStudentFiliere] = useState<Filiere | null>(null);
  ...
  const [moduleOptionsForEdit, setModuleOptionsForEdit] = useState<
  ...
  const fetchEvaluations = async (filiereId: number) => {
    try {
      ...
      setEvaluations(
        data.map((e: any) => ({ ...e, createdAt: new Date(e.createdAt) })),
      );
    } catch {
  ...
  const handleSyncEtudiants = async () => {
    try {
      ...
      // progression simulée pendant la synchronisation
      ...
      const result = await syncEtudiants();
      ...
      if (result.success) {
        toast.success(
          `${result.ajoutes} étudiants ajoutés, ${result.existants} déjà existants`,
        );

        // Recharge la liste locale
        const updatedStudents = await getEtudiants();
      ...
      console.error(error);
      toast.error("Erreur pendant le chargement des étudiants");
    } finally {
      setTimeout(() => {
        setIsSyncing(false);
        setSyncProgress(0);
  ...
  const handleAddEvaluation = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedFiliere || !selectedModule || !selectedEtudiant) {
  ...
  const openEditPopup = async (evaluation: Evaluation) => {
    ...
    try {
      ...
      const competencesWithScores: CompetenceScore[] = competences.map((c) => {
        ...
        const existingScore = evaluation.competences.find(
          (ec) => ec.competenceId === c.id,
        );
        return {
          competenceId: c.id,
          competenceNom: c.nom,
  ...
  const filteredEvaluations = evaluations.filter((e) => {
    ...
    return (
      ...
      e.filiere?.nom.toLowerCase().includes(query) ||
      e.module?.intitule.toLowerCase().includes(query)
    );
  });
        ...
        <dialog className="modal modal-open">
          ...
          >
            ...
            {/* <Select
              ...
              onChange={(o) => {
                const student = etudiants.find((e) => e.id === o?.value);

                setSelectedEtudiant(o);

                if (student?.filiere) {
                  const filiere = filieres.find(
                    (f) =>
                      f.nom.toLowerCase() === student.filiere?.toLowerCase(),
                  );
            ...
            <Select
              ...
              onChange={(o) => {
                ...
                if (!student?.filiere) {
                  toast.warning("Cet étudiant n'a pas de filière définie");
                  return;
                }

                const studentFiliere = normalizeText(student.filiere);

                const filiere = filieres.find(
                  (f) => normalizeText(f.nom) === studentFiliere,
                );
                ...
                if (filiere) {
                  ...
                  // Recharge les modules de cette filière
                  getModulesByFiliere(filiere.id)
                    .then((data) => {
                      setModules(data);
                  ...
                  toast.error(
                    `La filière "${student.filiere}" n'existe pas dans la liste des filières`,
                  );
                }
              }}
              placeholder="Sélectionner un étudiant"
            ...
            <Select
              options={filiereOptions}
              value={selectedFiliere}
              onChange={(o) => setSelectedFiliere(o as any)}
              placeholder="Sélectionner une filière"
            />
            <Select
              options={moduleOptions}
              value={selectedModule}
              onChange={(o) => setSelectedModule(o as any)}
              placeholder="Sélectionner un module"
            />

            {formScores.length === 0 ? (
              <p className="text-gray-500 text-sm">
                Sélectionner un module pour voir les compétences
              </p>
            ) : (
        ...
        <dialog className="modal modal-open">
          <div className="modal-box max-w-lg w-full p-6 flex flex-col gap-4">
            ...
            <p className="text-sm text-gray-500">
              Étudiant: {detailEvaluation.etudiant?.nom}{" "}
              {detailEvaluation.etudiant?.postnom}{" "}
              {detailEvaluation.etudiant?.prenom}
            </p>
            <p className="text-sm text-gray-500">
            ...
            <div className="mt-4 space-y-2">
              {detailEvaluation.competences.map((c) => (
                ...
                >
                  ...
                  <span>
                    {c.score?.toFixed(2) ?? 0} / {c.maxScore}
                  </span>
                </div>
              ))}
            </div>
            ...
            {(() => {
              const totalScore = detailEvaluation.competences.reduce(
                (sum, c) => sum + (c.score || 0),
                0,
              );
              const totalMax = detailEvaluation.competences.reduce(
      ...
      {editEvaluation && (
        <dialog className="modal modal-open">
            ...
            onSubmit={async (e) => {
              ...
              try {
                ...
                await updateEvaluation(
                  editEvaluation.id,
                  editEvaluation.competences.map((s) => ({
                    competenceId: s.competenceId,
                    score: s.score,
                  })),
                );
          ...
          >
            ...
            <p className="text-sm text-gray-500">
              Étudiant: {editEvaluation.etudiant?.nom}{" "}
              {editEvaluation.etudiant?.postnom}{" "}
              {editEvaluation.etudiant?.prenom}
            </p>
            <p className="text-sm text-gray-500">
              Filière: {editEvaluation.filiere?.nom}
            </p>
            ...
            <Select
              ...
              onChange={async (o) => {
                ...
                const existingIds = editEvaluation.competences.map(
                  (c) => c.competenceId,
                );
